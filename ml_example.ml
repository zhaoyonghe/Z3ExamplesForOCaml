...
let basic_tests ( ctx : context ) =
  ...
  let fname: symbol = (Symbol.mk_string ctx "f") in
  let x: symbol = (Symbol.mk_string ctx "x") in
  let y: symbol = (Symbol.mk_string ctx "y") in
  let bs: sort = (Boolean.mk_sort ctx) in
  let domain: sort list = [ bs; bs ] in
  let f: func_decl = (FuncDecl.mk_func_decl ctx fname domain bs) in
  let fapp: expr = (mk_app ctx f 
		[ (Expr.mk_const ctx x bs); (Expr.mk_const ctx y bs) ]) in
  let fargs2: expr list = [ (mk_fresh_const ctx "cp" bs) ] in
  let domain2: sort list = [ bs ] in
  let fapp2: expr = (mk_app ctx (mk_fresh_func_decl ctx "fp" domain2 bs) fargs2) in
  let trivial_eq: expr = (Boolean.mk_eq ctx fapp fapp) in
  let nontrivial_eq: expr = (Boolean.mk_eq ctx fapp fapp2) in
